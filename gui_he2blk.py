...
#################TODO#################
# During landmark, reload images, should not keep the points
...
class ImageSelector(QMainWindow):
    ...
    def load_image1(self):
        ...
        self.image1_path, _ = QFileDialog.getOpenFileName(self, "Open Block Face Image", "", "Images (*.png *.jpg *.jpeg *.bmp *.tiff);;All Files (*)", options=options)
        pixmap_1 = QPixmap(self.image1_path)

        # half_width = pixmap_1.width() // 2
        # half_height = pixmap_1.height() // 2
    ...
    def load_image2(self):
        options = QFileDialog.Options()
        self.image2_path, _ = QFileDialog.getOpenFileName(self, "Open Histology Image", "", "Images (*.png *.jpg *.jpeg *.bmp *.tiff);;All Files (*)", options=options)
    ...
    def load_landmarks(self):
        options = QFileDialog.Options()
        _landmarks_file, _ = QFileDialog.getOpenFileName(self, "Load Landmarks *.npy", "*.npy", options=options)
    ...
    def save(self):
        options = QFileDialog.Options()
        filename, _ = QFileDialog.getSaveFileName(self, "Save transformed image, *.png", ".png", options=options)
        if filename:
    ...
    def rigid(self):
        if len(self.points_image1) == len(self.points_image2):
            ...
            pts_img1 = np.array([[point.x(), point.y()] for point in self.points_image1]) * 2
    ...
    def thin_plate(self):
        if len(self.points_image1) == len(self.points_image2):
            # print("Number of points selected on both images is equal.")
            # print("Points in image 1:", self.points_image1)
            # print("Points in image 2:", self.points_image2)
            ...
            # transform_func = tps_transform(pts_img2, pts_img1)
            ...
            # print(pts_img1)
            # print(pts_img2)
            # f32_pts = np.zeros(pts_img1.shape, dtype=np.float32)
            # f32_pts[:] = pts_img1[:]
            # print(pts_img1.shape)
            # matrix = splines.applyTransformation(f32_pts)
            # print(transform_matrix)
            # quit()

            # self.output_image=splines.warpImage(img2) 
        ...
        #########################
            ...
            # _h = h if h>height else height
            # _w = w if w>width else width

            # print(height, width)

            # Create an empty output image
            self.output_image = np.zeros_like(img1)

            # Apply TPS transformation to each pixel
            # self.apply_transformation_optimized(img2, self.output_image, splines)
            # for y in range(h):
            #     for x in range(w):
            #         f32_pts = np.array([[[x,y]]], dtype=np.float32)
            ...
            #         # print(tx,ty)
                    ...
                    #     # Use bilinear interpolation to get the RGB values
                    ...
                    #     dx, dy = tx - x0, ty - y0
                    #     try:
                    #         pixel = (1 - dx) * (1 - dy) * img2[y0, x0] + dx * (1 - dy) * img2[y0, x1] + \
                    #             (1 - dx) * dy * img2[y1, x0] + dx * dy * img2[y1, x1]
                    #         self.output_image[y, x] = pixel
            ...
            #     0 <= tx < w, 0 <= ty < h
            # ))
            _valid_indices = np.logical_and(np.logical_and(0 <= tx, tx < width-1), np.logical_and(0 <= ty, ty < height-1))

            print(img1.shape, img2.shape)

            pixel = np.zeros_like(self.output_image).reshape(-1, 3)

            if(pixel.shape[0]>len(_valid_indices)):            
                valid_indices = np.full(pixel.shape[0], False)
                valid_indices[:len(_valid_indices)] = _valid_indices
            else:
            ...
            pixel[valid_indices] = (
                ((1 - dx[valid_indices]) * (1 - dy[valid_indices]))[:, np.newaxis] * img2[y0[valid_indices], x0[valid_indices]] +
                (dx[valid_indices] * (1 - dy[valid_indices]))[:, np.newaxis] * img2[y0[valid_indices], x1[valid_indices]] +
                ((1 - dx[valid_indices]) * dy[valid_indices])[:, np.newaxis] * img2[y1[valid_indices], x0[valid_indices]] +
                (dx[valid_indices] * dy[valid_indices])[:, np.newaxis] * img2[y1[valid_indices], x1[valid_indices]]      )

            

            self.output_image = pixel.reshape(h,w,3)

            if(self.checkbox.isChecked()):
                img_an = cv2.imread(self.image2_path.split('he.png')[0]+'labels.png')
                if(img_an.shape == img2.shape):
                    print(self.image2_path.split('he.png')[0]+'label.png')
                    self.output_an = np.zeros_like(img1)
                    ...
                    pixel[valid_indices] = (
                        ((1 - dx[valid_indices]) * (1 - dy[valid_indices]))[:, np.newaxis] * img_an[y0[valid_indices], x0[valid_indices]] +
                        (dx[valid_indices] * (1 - dy[valid_indices]))[:, np.newaxis] * img_an[y0[valid_indices], x1[valid_indices]] +
                        ((1 - dx[valid_indices]) * dy[valid_indices])[:, np.newaxis] * img_an[y1[valid_indices], x0[valid_indices]] +
                        (dx[valid_indices] * dy[valid_indices])[:, np.newaxis] * img_an[y1[valid_indices], x1[valid_indices]])


                    self.output_an = pixel.reshape(h,w,3)

                else:
                    print("\033[31mThe shape of H&E and Annotation are not equal!\033[0m")
